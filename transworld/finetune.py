...
def run(scenario, test_data, training_step, pred_step, hid_dim, n_heads, n_layer, device):
    ...
    exp_dir = Path(__file__).parent.parent / "HighD"

    exp_setting = exp_dir / "highway02"
    preTrainModel = exp_setting / 'preTrainModel'
    FineTuneModel = exp_setting / 'FineTuneModel'
    ...
    out_dir = preTrainModel / f"out_dim_{hid_dim}_n_heads_{n_heads}_n_layer_{n_layer}_pred_step_{pred_step}"
    ...
    hid_dim = hid_dim
    n_heads = n_heads
    changable_feature_names = ['xAcceleration','yAcceleration']
    ...
    encoder_path = preTrainModel / 'encorder.pth'
    generator_path = preTrainModel / 'generator.pth'
    ...
    # torch.save(encoder.state_dict(), out_dir / 'encorder_cali.pth')
    # torch.save(generator.state_dict(), out_dir / 'generator_cali.pth')
...
if __name__ =="__main__":
    ...
    parser.add_argument("--training_step", type=int, default=1100)
