...
def load_graph(data_path: Path, start_step, end_step,node_id_dict) -> Tuple[Dict, Dict]:
    ...
    file_names = ["node", "edge", "feat_veh", "feat_lane"]
    feat_file_names = ["feat_veh", "feat_lane"]
    #except_columns = ['name', 'coordinate',"shape"]
    scalers = {'veh': MinMaxScaler(), 'lane': MinMaxScaler()}
    ...
    for data_file in all_files:
        ...
        if df_name == "edge":
            ...
            dfs[df_name]["node_id"] = [
                node_id_dict[i] for i in dfs[df_name]["name"]
            ]
        else:
            pass
    ...
    feat_dict = generate_feat_dict("veh", dfs["feat_veh"], start_step, end_step)
    feat_dict.update(generate_feat_dict("lane", dfs["feat_lane"], start_step, end_step))
